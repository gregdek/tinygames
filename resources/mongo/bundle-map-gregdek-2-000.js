...

{
	...
	setObject:[
		...
		
		{
                        ...
                        property:(tilemaps.queststatus["gregdek-2-002complete"]?"intro":"notintro"),
		...
		// Map data and actions
		{
			...
			value:{
				...
				addObjects:function() {
					...
					if (tilemaps.queststatus["gregdek-2-000complete"]) {

						// if yes, but the user hasn't finished the whole quest,
						// add the stairs.

						if (!tilemaps.queststatus["gregdek-2-002complete"]) {
						...
						// user hasn't completed gregdek-2-000, add the trigger.
				...
				mapActions:function() {
					...
					// On stairs?  Go to the next level.
                                        if (ontile==203) { maingame.gotoLevel({level:"gregdek-2-001",x:90,y:90,introdialogue:true,label:"Five Alive"}); }

					// On trigger?  Create the stairs.

					if ((ontile==202) && (!tilemaps.queststatus["gregdek-2-000complete"])) {
