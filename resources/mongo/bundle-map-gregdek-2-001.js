		...
		// First, set dialogues for NPC(s).
		{
                        ...
                        // value:{ font:"smalltut", skipkey:"a", who: noface,
                                scenes:[
                                        ...
                                        { speed:1, who:"noone", audio:"beep", talk:["Walk over only those numbers","that are divisible by 5!"]},
                                        { speed:1, who:"noone", audio:"beep", talk:["Then go to the red button","and the stairs will appear!"]}
                                ]
                        }
                },

                {
                        ...
                        value:{
                                ...
                                scenes:[
                                        { speed:1, who:"noone", audio:"beep", talk:["What numbers are","divisible by 5?"]},
                                        { speed:1, who:"noone", audio:"beep", talk:["Any number that ends in:"]},
                                        { speed:1, who:"noone", audio:"beep", talk:["0 or 5!"]}
                                ]
                        }
                },
				...
				mapActions:function() {
					...
					if ((ontile>=0) && (ontile<100)) { 
						maingame.setTileInMap(xc,yc,ontile+100,true);
                                        ...
                                        if (ontile==203) { maingame.gotoLevel({level:"gregdek-2-002",x:90,y:90,introdialogue:true,label:"Three-dom for Freedom!"}); }
					...
					if ((ontile==202) && (!tilemaps.queststatus["gregdek-2-001complete"])) {

						// 202 is the red button that triggers the puzzle test, until the quest is cleared.  
						// 
						// Our puzzle solution is "all/only numbers divisible by 5 triggered."  Which
						// means that if we find any %5 tiles < 100, or any !(%5) tiles
						...
						for (var ty=0; ty<tilemaps.map.map.length; ty++) {
							for (var tx=0; tx<tilemaps.map.map[ty].length; tx++) {
								...
								if (
									// If a solid tile is %5 ...
									(((tilenum>=100) && (tilenum<200)) && (tilenum % 5 != 0)) ||
									// Or if a non-solid tile is %5 ...
									(((tilenum>=0) && (tilenum<100)) && (tilenum % 5 == 0)) 
								) {
									// then fail it!
						...
						if (tilemaps.queststatus["_tmpgregdek-2-001fail"]==true) {
							...
							tilemaps.queststatus["gregdek-2-001complete"] = true;
						}
					}
				},
