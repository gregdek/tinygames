		...
		// First, set dialogues for NPC(s).
		{
                        ...
                        // value:{ font:"smalltut", skipkey:"a", who: noface,
                                scenes:[
                                        ...
                                        { speed:1, who:"noone", audio:"beep", talk:["Walk over only the even numbers","on your way to the red button..."]},
...

                {
                        ...
                        value:{
                                ...
                                scenes:[
                                        { speed:1, who:"noone", audio:"beep", talk:["What is an even number?"]},
                                        { speed:1, who:"noone", audio:"beep", talk:["It's a number that ends in:"]},
                                        { speed:1, who:"noone", audio:"beep", talk:["0, 2, 4, 6, 8!"]}
                                ]
                        }
                },
				...
				mapActions:function() {
					...
					if ((ontile>=0) && (ontile<100)) { 
						maingame.setTileInMap(xc,yc,ontile+100,true);
                                        ...
                                        if (ontile==203) { maingame.gotoLevel({level:"gregdek-1-000",x:90,y:90,introdialogue:true,label:"Quest Complete!"}); }
					...
					if ((ontile==202) && (!tilemaps.queststatus["gregdek-1-002complete"])) {

						// 202 is the red button that triggers the puzzle test, until the quest is cleared.  
						// 
						// Our puzzle solution is "all even numbers triggered."  Which
						// means that if we find any even tiles < 100, or any odd tiles
						...
						for (var ty=0; ty<tilemaps.map.map.length; ty++) {
							for (var tx=0; tx<tilemaps.map.map[ty].length; tx++) {
								...
								if (
									// If a solid tile is odd...
									(((tilenum>=100) && (tilenum<200)) && (tilenum % 2 == 1)) ||
									// Or if a non-solid tile is even...
									(((tilenum>=0) && (tilenum<100)) && (tilenum % 2 == 0)) 
								) {
									// then fail it!
						...
						if (tilemaps.queststatus["_tmpgregdek-1-001fail"]==true) {
							...
							tilemaps.queststatus["gregdek-1-002complete"] = true;
						}
					}
				},
